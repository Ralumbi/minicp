...
class WifiManagerFrame(tk.Frame):
    ...
    def build_ui(self):
        # Adapter selector
        tk.Label(self, text="Adapter:", font=("Arial", 10)).grid(row=0, column=0, sticky="w", padx=5, pady=5)
        ...
        tk.Button(self, text="Refresh", command=self.refresh_ifaces, font=("Arial", 10), width=10, height=1)\
            .grid(row=0, column=2, sticky="ew", padx=5, pady=5)

        self.refresh_ifaces()

        # Scan button and list
        tk.Button(self, text="Scan", command=self.scan, font=("Arial", 10), width=10, height=1)\
            .grid(row=1, column=0, padx=5, pady=5)
        self.lst = tk.Listbox(self, height=3, font=("Arial", 10))
        ...
        # SSID & PSK entries
        tk.Label(self, text="SSID:", font=("Arial", 10)).grid(row=3, column=0, sticky="w", padx=5, pady=5)
        self.ssid_entry = tk.Entry(self, font=("Arial", 10))
        self.ssid_entry.grid(row=3, column=1, columnspan=2, sticky="ew", padx=5, pady=0)
        self.ssid_entry.bind('<FocusIn>', lambda e: self.open_keyboard(self.ssid_entry))

        tk.Label(self, text="Password:", font=("Arial", 10)).grid(row=4, column=0, sticky="w", padx=5, pady=5)
        ...
        tk.Button(self, text="Connect", bg="green", fg="white", command=self._connect, font=("Arial", 10), width=10, height=1)\
            .grid(row=4, column=2, padx=5, pady=5)
        tk.Button(self, text="Disconnect", command=lambda: self.app.wifi_mgr.disconnect(self.iface_var.get()), font=("Arial", 10), width=10, height=1)\
    ...
    def scan(self):
        ...
        for net in nets:
            ...
            print(f"Network: {net['ssid']}, Signal: {net['signal']}%")
